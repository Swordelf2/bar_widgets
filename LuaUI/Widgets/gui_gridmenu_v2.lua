...
local function drawCell(rect, cmd, usedZoom, cellColor, disabled)
	...
	if cmd.hotkey and (builderIsFactory or (activeBuilder and currentCategory)) then
		...
		local keyFontSize = priceFontSize * 1.2
...
local function drawGrid()
	...
	-- draw detailed info next to cursor TODO: move to separate widget
	if activeCmd ~= nil and config.showInfoUnderCursor then
		local x, y, _, _, _ = Spring.GetMouseState()
		if selectedCell ~= nil then
			DrawDetailedCostLabels(x+10, y, selectedCell, 2*priceFontSize)
		end
	end
end
...
function widget:DrawScreen()
	...
	if not (isPregame or activeBuilder or alwaysShow) then
		...
		local now = os.clock()
		-- TODO: remove activecmd from here when moved under cursor info display
		if doUpdate or activeCmd or (doUpdateClock and now >= doUpdateClock) then
