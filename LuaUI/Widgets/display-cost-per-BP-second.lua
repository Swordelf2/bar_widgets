...
function widget:GetInfo()
	return {
		name = "Display cost per BP second",
		desc = "Build menu with grid hotkeys. Displays cost per second",
		author = "Floris, grid by badosu and resopmok. Cost per second by Engolianth",
		date = "October 2023",
...
local units = VFS.Include("LuaUI/Config/unit_buildmenu_config.lua")
...
local function drawCell(rect, cmd, usedZoom, cellColor, disabled)
    ...
    local metalFor100BuildPowerSecond = math.floor(units.unitMetalCost[uid] / units.unitBuildTime[uid] * 100 + 0.5)
		local energyColor = disabled and "\255\135\135\135" or "\255\255\255\000"
    local energyFor100BuildPowerSecond = math.floor(units.unitEnergyCost[uid] / units.unitBuildTime[uid] * 100 + 0.5)
		local text = metalColor .. metalFor100BuildPowerSecond .. "\n" .. energyColor .. energyFor100BuildPowerSecond .. "\n\n" .. metalColor .. units.unitMetalCost[uid] .. "\n" .. energyColor .. units.unitEnergyCost[uid]

		font2:Print(
      text, 
      rect.x + cellPadding + (cellInnerSize * 0.048),
      rect.y + cellPadding + (priceFontSize * 1.35 * 3.2),
      priceFontSize,
      "o")
	end
