...
function widget:GetInfo()
    return {
        name = "Attack Range GL4 mine",
        desc = "[v" .. string.format("%s", versionNumber) ..
...
local function initializeUnitDefRing(unitDefID)
    ...
    for weaponNum = 1, #weapons do
        ...
        if weaponType ~= nil and weaponType > 0 then
            ...
            local ringParams = {range, color[1], color[2], color[3], color[4], fadeparams[1], fadeparams[2],
                                fadeparams[3], fadeparams[4], weaponDef.projectilespeed or 1, isCylinder,
                                weaponDef.heightBoostFactor or 0, weaponDef.heightMod or 0, groupselectionfadescale,
                                weaponType, isDgun}
    ...
    if unitBuilder[unitDefID] then
        ...
        local ringParams = {range, color[1], color[2], color[3], color[4], fadeparams[1], fadeparams[2], fadeparams[3],
                            fadeparams[4], 1, false, 0, 0, groupselectionfadescale, 2, 0}
...
local circleInstanceVBOLayout = {{
    ...
    name = 'posscale',
    size = 4
}, -- a vec4 for pos + scale
{
    id = 2,
    name = 'color1',
    size = 4
}, --  vec4 the color of this new
{
    id = 3,
    name = 'visibility',
    size = 4
}, --- vec4 heightdrawstart, heightdrawend, fadefactorin, fadefactorout
{
    ...
    size = 4
}, --- heightboost gradient
{
    id = 5,
    name = 'additionalParams',
    size = 4
}, --- groupselectionfadescale, +3 additional reserved
{
    id = 6,
    name = 'instData',
    size = 4,
...
local function makeCircleVBO(circleSegments)
    ...
    if circleVBO == nil then
        goodbye("Failed to create circleVBO")
    end
    ...
    circleVBO:Define(circleSegments + 1, VBOLayout)
...
local function initGL4()
    ...
    for i, atkRangeClass in ipairs(attackRangeClasses) do
        attackRangeVAOs[atkRangeClass] = makeInstanceVBOTable(circleInstanceVBOLayout, 20,
            atkRangeClass .. "_attackrange_gl4", 6) -- 6 is unitIDattribID (instData)
        ...
        local newVAO = makeVAOandAttach(attackRangeVAOs[atkRangeClass].vertexVBO,
...
local function CycleUnitDisplay(direction)
    ...
    Spring.Echo("Changed range display of " .. name .. " to config " .. tostring(bitmap) .. ": " ..
